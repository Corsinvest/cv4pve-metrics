...
if ( $CV4PVE_AUTOSNAP_PHASE -eq "snap-create-abort" -or $CV4PVE_AUTOSNAP_PHASE -eq "snap-create-post" ) {
    success = ($CV4PVE_AUTOSNAP_PHASE -eq "snap-create-post")

    #url post
    url="http://$INFULX_DB_HOST\:$INFULX_DB_PORT/write?db=$INFULX_DB_NAME"

    #data metrics
    data="autosnap,host=$HOSTNAME,vmid=$CV4PVE_AUTOSNAP_VMID,type=$CV4PVE_AUTOSNAP_VMTECHNOLOGY host=$HOSTNAME,vmid=$CV4PVE_AUTOSNAP_VMID,type=$CV4PVE_AUTOSNAP_VMTECHNOLOGY,success=$success"

    if ( $INFULX_DB_USER -eq "" ) {
        #no login
